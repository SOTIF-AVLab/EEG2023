...
model1 = EEGNet(nb_classes = 2, Chans = 63, Samples = 750)
model2 = ShallowConvNet(nb_classes = 2, Chans = 63, Samples = 750)
model3 = DeepConvNet(nb_classes = 2, Chans = 63, Samples = 750)
...
for subject in subject_number:
    ...
    for event_Number in eventNumber:
        ...
        fittedModel = model1.fit(x_train, y_train, batch_size=16, epochs=300, verbose=2, validation_split=0.2)
        predicted = model1.predict(x_test, y_test)
        print(predicted)